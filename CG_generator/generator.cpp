...
//-----------------------------------
	...
	std::normal_distribution<double> distribution(300, 10);
	...
	char ch_error[20];
	int gap_1, gap_2, gap_3, gap_4, gap_5, gap_6, gap_7;
	int base;
	int loc;
...
//	int no_ch = countRefSeq("../reference/build37.fa", ch_name_array, ch_count_array,
...
//	makeRefSeq("../reference/build37.fa", ch_array, ch_array_error);
...
//----------------------------------------------
				if (ErrSeqForward == 0) {
					randErr (5, tmp_no_error, storageForward);
				}
				else {
					randErr (10, tmp_no_error, storageForward);
				}
				if (ErrSeqReverse == 0) {
					randErr (5, tmp_no_error, storageReverse);
				}
				else {
			...
			//------------------------------------------------------------
			printf("	######################################################################");
			printf("	Loc: %d Gap: %d %d %d %d %d %d %d ",
					base, gap_1, gap_2, gap_3, gap_4, gap_5, gap_6, gap_7);
